...
from cs336_basics.Transformer import learning_rate_schedule, gradient_clipping, data_loading
from cs336_basics.Transformer import checkpoint, load_checkpoint
def run_linear(
    d_in: int,
    d_out: int,
...
def run_swiglu(
    ...
    stat_dict = {"W1.W": w1_weight, "W2.W": w2_weight, "W3.W": w3_weight,}
    model.load_state_dict(stat_dict)
    return model(in_features)
...
def run_multihead_self_attention(
    ...
    """
    d_weight = {'k':k_proj_weight, 'q':q_proj_weight, 'v':v_proj_weight, 'o':o_proj_weight}
    multihead_model = multihead_self_attention(d_model, num_heads, d_weight)
    return multihead_model(in_features)
...
def run_multihead_self_attention_with_rope(
    ...
    d_weight = {'k':k_proj_weight, 'q':q_proj_weight, 'v':v_proj_weight, 'o':o_proj_weight}
    multihead_model = multihead_self_attention(d_model, num_heads, d_weight, RoPE, token_positions)
    return multihead_model(in_features)
...
def run_transformer_block(
    ...
    """
    w = dict()
    w["MHA"] = {'k':weights["attn.k_proj.weight"], 'q':weights["attn.q_proj.weight"],
                'v':weights["attn.v_proj.weight"], 'o':weights["attn.output_proj.weight"]}
    w["ff"] = {'W1.W': weights["ffn.w1.weight"], 'W2.W': weights["ffn.w2.weight"],
               'W3.W': weights["ffn.w3.weight"]}
    w["RMS1"] = {'gamma': weights["ln1.weight"]}
    w["RMS2"] = {'gamma': weights["ln2.weight"]}
    RoPE = RotaryPositionalEmbedding(theta, int(d_model/num_heads), max_seq_len)
    Block = transformer_block(d_model, num_heads, d_ff, None, RoPE, None, w)
...
def run_rmsnorm(
    ...
    stat_dict = {"gamma": weights}
    RMS.load_state_dict(stat_dict)
...
def run_save_checkpoint(
    ...
    """
    checkpoint(model, optimizer, iteration, out)
