...
def train_bpe(
    ...
    **kwargs,
    ...
    for token_ids, count in dic_word.items():
        for i in range(len(token_ids)-1):
            pair = (token_ids[i], token_ids[i+1])
            dic_pair[pair] += count
            pair_to_words[pair].add(token_ids)
    dic_pair = dict(dic_pair)
    t2 = time.time()
    print(f"构建pair词典用时：{t2-t1:.4f}s")
    ...
    while current_id < max_num:
        # finf the most frequent pair
        if not dic_pair: 
            break
        max_key = max(dic_pair, key=lambda k: (dic_pair[k], vocab[k[0]], vocab[k[1]]))
        merges.append((vocab[max_key[0]], vocab[max_key[1]]))
        vocab[current_id] = vocab[max_key[0]] + vocab[max_key[1]]

        # list copying used to avoid problems caused by modifying the set during iteration
        affected_words = list(pair_to_words[max_key].copy())
        pair_to_words[max_key].clear()

        for old_token_ids in affected_words:
            if old_token_ids not in dic_word:
                continue
            count = dic_word[old_token_ids]

            new_token_ids, modified = merge(max_key, old_token_ids, current_id)
            if not modified:
                continue

            # update dic_word
            del dic_word[old_token_ids]
            dic_word[new_token_ids] = dic_word.get(new_token_ids, 0) + count

            # remove influence on pair count from old vocabs
            # delete influenced pair in pair_to_words dict
            for i in range(len(old_token_ids)-1):
                old_p = (old_token_ids[i], old_token_ids[i+1])
                dic_pair[old_p] -= count
                if dic_pair[old_p] <= 0:
                    del dic_pair[old_p]
                if old_token_ids in pair_to_words[old_p]:
                    pair_to_words[old_p].remove(old_token_ids)

            # add new count
            # update pair_to_words
            for i in range(len(new_token_ids)-1):
                new_p = (new_token_ids[i], new_token_ids[i+1])
                dic_pair[new_p] = dic_pair.get(new_p, 0) + count
                pair_to_words[new_p].add(new_token_ids)

        if max_key in dic_pair:
            del dic_pair[max_key]

        current_id += 1
        if current_id % 100 == 0:
            print(f"已合并 {current_id}/{max_num}，用时：{time.time()-t1:.4f}s")

    # add special tokens
    for i, t in enumerate(special_tokens):
        vocab[current_id + i] = t.encode("utf-8")
    return vocab, merges
...
class Tokenizer():
    def __init__(self,
        ...
        self.vocab_dict = {k: v for v, k in self.vocab.items()}
        self.pair_to_id = {(self.vocab_dict[a], self.vocab_dict[b]): self.vocab_dict[a+b] 
                          for a, b in merges}
        self.merge_pair_list = list(self.pair_to_id.keys())
    @classmethod
    def from_files(cls,
                   vocab_filepath: str,
                   merges_filepath: str,
        ...
        for token_ids in result:
            while True:
                ...
                for i, pair in enumerate(pairs):
                    if pair in self.pair_to_id and self.merge_pair_list.index(pair) <= index:
                        index = self.merge_pair_list.index(pair)
                        merge_id = self.pair_to_id[pair]
                        merge_pos = i
                
                if merge_id is None:
                    break 
                token_ids[merge_pos] = merge_id
                del token_ids[merge_pos + 1]
...
if __name__ == "__main__":
    ...
    np.save("TinyStories_token_ids_valid.npy", token_array)